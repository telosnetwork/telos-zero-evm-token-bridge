...
export async function loadConfig(path) {
  ...
  for (const pair of config.pairs) {
    ...
    if (!Number.isInteger(pair.evmDecimals) || pair.evmDecimals < 0) {
      throw new Error(`pair ${pair.pairId} evmDecimals must be a non-negative integer`);
    }
    if (!Number.isInteger(pair.zeroDecimals) || pair.zeroDecimals < 0) {
      throw new Error(`pair ${pair.pairId} zeroDecimals must be a non-negative integer`);
    }
  }
